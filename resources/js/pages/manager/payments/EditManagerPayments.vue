...
const getData = () => {
  axios.get(`/payments/+${router.currentRoute.value.params.id}/edit`)
       .then((response)=>{
...
const editFunction = (values, actions) => {
  ...
  axios.patch(`/payments/${retrievedData.value.id}`,values).then((response)=>{

    // managers.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    actions.resetForm();
    router.push({ path: '/manager/payments' });
    toastr.success('Pagamento editada com sucesso');
...
<template>
    <div v-if="!loadingDiv">
        ...
        <h1 class="h3 mb-3">Pagamento </h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Pagamento: {{ retrievedData.name }}</h5>

                                        <router-link to="/manager/payments" class="btn btn-pill btn-primary mt-3"><vue-feather type="arrow-left"></vue-feather>Voltar</router-link> 
